...
${context.options.plugins?.map((plugin) => `import ${plugin} from "quasar/${context.imports.raw[plugin]}"`).join("\n") || ""}
...
${when(ssr && isServer, () => `
  ...
  nuxt.vueApp.use(Quasar, {
    ...
    plugins: {${when(ssr, "NuxtPlugin, ") + (context.options.plugins?.join(",") || "")}},
...
function resolveQuasarModuleSideEffectsPlugin() {
  return {
...
function muteQuasarSassWarnings(config) {
  var _a;
...
export { module as default, muteQuasarSassWarnings, setupCss };
